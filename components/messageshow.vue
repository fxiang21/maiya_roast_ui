  ...
  <view>
    <view class="m-item m-item_flag" ref="nessageChild" :id="'message' + id">
      <view class="m-left" :style="{ width: leftWidthReal }">
        <image class="head_icon" src="@/static/img/maiya.png" v-if="message.user === 'home'"></image>
      </view>
      
      <view class="m-content" v-if="!message.loading"
            :style="{ paddingBottom: currentMax < id + 1 && message.user === 'home' ? '0px' : '0px' }">
        ...
        <view class="m-content-head" :class="{'m-content-head-right': message.user === 'customer'}">
          <view v-if="message.user === 'customer'" :class="'m-content-head-' + message.user"
                @longpress="copy(message.user)">{{ message.content }}</view>
          
          <view v-if="message.user === 'home'" :class="'m-content-head-' + message.user"
                @longpress="copy(message.user)" :style="{ paddingBottom: voiceOpen ? '10px' : '' }">
            <ua-markdown :source="message.content"  :width="calculatedWidth" :showLine="false"></ua-markdown>
            <view v-if="voiceOpen" style="display: flex; align-items: center;">
              <br />
              <uni-icons
                style="margin-top: 5px;"
                :type="play ? 'sound-filled' : 'sound'"
                @click="playVoice"
                size="30">
              </uni-icons>
              <wave-animation-up style="margin-left: 10px;" :play="play" :getVoiceData="getVoiceData"></wave-animation-up>
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="m-right" v-if="!message.loading" :style="{ width: rightWidthReal }">
        ...
        <view class="copy-parent" v-if="message.user === 'home'"></view>
      </view>
    ...
    <view v-if="currentMax === id + 1 && guideInfo !== null" class="m-item_flag" style="padding-bottom: 60px; padding-top: 40px;">
      <view class="guide">
        <view class="guide_title" v-if="guideInfo.title !== ''">{{ guideInfo.title }}</view>
        <view class="guide_text" v-if="guideInfo.content !== ''">{{ guideInfo.content }}</view>
        <view class="guide_select clickEacct" 
              v-for="(item, index) in guideInfo.selectList" 
              :key="index" 
              @click="inputSelect(item)">
          <text>{{ item }}</text>
	...
	export default {
		data() {
			return {
				...
				myIcon: getApp().globalData.isBlank(uni.getStorageSync(getApp().globalData.avatarUrlCacheName)) ?
					'../static/img/me.png' : uni.getStorageSync(getApp().globalData.avatarUrlCacheName),
				guideInfo: null,
		...
		methods: {
			...
			initGuideInfo: function() {
				console.log("initGuideInfo:1", this.id, this.currentMax)
				if (this.id + 1 === this.currentMax && this.guideInfo === null) {
					//最后显示，可以添加引导元素
					console.log("initGuideInfo:2")
					uni.http.guide(null, (res) => {
						console.log("查询到数据", res);
						console.log("查询到数据2", res.data);
						console.log("查询到数据3", res.data.data);
